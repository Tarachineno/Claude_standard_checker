...
現在は`static/catalog.js`の`CATALOG_EDITING_UI_ENABLED = false`で画面からの編集を一時停止している。「確認・補完」「自動取得に戻す」と管理キー・メールログイン・保存フォームを非表示にし、確認履歴・公式出典・比較・CSV出力は維持する。認証APIや定期巡回、既存データは変更しない。この表示スイッチは認証の代わりではない。

以下のAccess設定を整えた後、所有者の承認を得てこのスイッチを`true`に戻してDeployし、受信・ログイン・保存を確認する。HTMLの`hidden`は初期表示のちらつき防止なので残す（初期化時にスイッチへ追従する）。
