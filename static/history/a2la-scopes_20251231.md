# A2LA Certificate Scope Information

**Certificate Number:** A2LA-2022-01  
**Organization:** A2LA Accredited Testing Laboratory  
**Valid Until:** 2025-12-31  
**Accreditation Body:** A2LA  

## Test Categories

### Radiated & Conducted {#radiated-conducted}

- **CFR 47 FCC Part 15B (ANSI C63.4:2014)** - Unintentional Radiators
- **FCC Part 18 (MP-5:1986)** - Industrial, Scientific, and Medical Equipment
- **FCC Parts 15C (ANSI C63.10:2013)** - Intentional Radiators
- **FCC Part 15E (ANSI C63.10:2013 & FCC KDB 905462 D02 v02)** - U-NII Equipment
- **FCC Parts 15F (ANSI C63.10:2013)** - Ultra-Wideband Operation
- **UNII-MP** - Unlicensed National Information Infrastructure
- **CISPR 11** - Industrial, scientific and medical equipment
- **EN 55011** - Industrial, scientific and medical equipment
- **KS C 9811** - Korean EMC Standard
- **IEC 61000-6-4** - Generic emission standard for industrial environments
- **EN 61000-6-4** - Generic emission standard for industrial environments
- **KS C 9610-6-4** - Korean generic emission standard

### United States Radio {#us-radio}

- **CFR 47 FCC Parts 25, 30, 74, 90 (>3 GHz), 95 (>3 GHz), 97 (>3 GHz) & 101 (ANSI C63.26:2015)** - Radio Service Rules

### Canada Radio {#canada-radio}

- **ICES-Gen** - General EMC requirements
- **ICES-003** - Information Technology Equipment
- **RSS-GEN** - General Requirements for Radio Equipment
- **RSS-210** - Low-power Licence-exempt Radio Communication Devices
- **RSS-215** - Wireless Microphones
- **RSS-220** - Devices Using Ultra-Wideband Technology
- **RSS-247** - 2.4 GHz Band Spread Spectrum Equipment
- **RSS-248** - 5 GHz Band Equipment
- **RSS-251** - Fixed Wireless Access Equipment

### European Radio {#european-radio}

- **ETSI EN 301 091-1/-2/-3** - Electromagnetic compatibility and Radio spectrum Matters (ERM)
- **EN 301 783** - Land Mobile Service
- **EN 301 893** - 5 GHz high performance RLAN
- **EN 302 065-1/-2/-3/-4** - Short Range Devices
- **EN 302 264** - Meteor Burst Communications
- **EN 305 550-1/-2** - Direct Sequence Spread Spectrum (DSSS)
- **EN 300 328** - 2,4 GHz wideband transmission systems
- **EN 300 330** - Short Range Devices
- **EN 300 220-1/-2** - Short Range Devices
- **EN 303 413** - Satellite Earth Stations and Systems

### Australia / New Zealand Radio {#aus-nz-radio}

- **AS/NZS 4268** - Radio equipment and systems

### Emissions for Ports {#emissions-for-ports}

- **CISPR 32** - Electromagnetic compatibility of multimedia equipment
- **EN 55032** - Electromagnetic compatibility of multimedia equipment

### Harmonic Current Emissions {#harmonic-current-emissions}

- **IEC 61000-3-2** - Limits for harmonic current emissions
- **EN 61000-3-2** - Limits for harmonic current emissions

### Voltage Fluctuations & Flicker {#voltage-fluctuations-flicker}

- **IEC 61000-3-3** - Voltage fluctuations and flicker
- **EN 61000-3-3** - Voltage fluctuations and flicker
- **IEC 61000-3-11** - Voltage fluctuations and flicker
- **EN 61000-3-11** - Voltage fluctuations and flicker

### Electrostatic Discharge (ESD) {#electrostatic-discharge}

- **IEC 61000-4-2** - Electrostatic discharge immunity test
- **EN 61000-4-2** - Electrostatic discharge immunity test
- **KS C 9610-4-2** - Korean ESD immunity test

### RF Radiated EM Field Immunity {#rf-radiated-immunity}

- **IEC 61000-4-3** - Radiated electromagnetic field immunity test
- **EN 61000-4-3** - Radiated electromagnetic field immunity test
- **KS C 9610-4-3** - Korean RF radiated immunity test

### Electrical Fast/Transient Burst (EFT) {#electrical-fast-transient}

- **IEC 61000-4-4** - Electrical fast transient immunity test
- **EN 61000-4-4** - Electrical fast transient immunity test
- **KS C 9610-4-4** - Korean electrical fast transient immunity test

### Surge {#surge}

- **IEC 61000-4-5** - Surge immunity test
- **EN 61000-4-5** - Surge immunity test
- **KS C 9610-4-5** - Korean surge immunity test

### Conducted Immunity {#conducted-immunity}

- **IEC 61000-4-6** - Conducted RF immunity test
- **EN 61000-4-6** - Conducted RF immunity test
- **KS C 9610-4-6** - Korean conducted RF immunity test

### Transients & Surges (Vehicle) {#vehicle-transients}

- **ISO 7637-2** - Road vehicles electrical disturbances

### Magnetic Field Immunity {#magnetic-field-immunity}

- **IEC 61000-4-8** - Power frequency magnetic field immunity test
- **EN 61000-4-8** - Power frequency magnetic field immunity test
- **KS C 9610-4-8** - Korean magnetic field immunity test

### Voltage Dips/Interruptions/Variations {#voltage-dips-interruptions}

- **IEC 61000-4-11** - Voltage dips, short interruptions and voltage variations immunity test
- **EN 61000-4-11** - Voltage dips, short interruptions and voltage variations immunity test
- **KS C 9610-4-11** - Korean voltage dips immunity test
- **KS C IEC 61000-4-34** - Korean voltage dips and interruptions test
- **IEC 61000-4-34** - Voltage dips, short interruptions and voltage variations test
- **EN 61000-4-34** - Voltage dips, short interruptions and voltage variations test

### Semiconductor Processing Equipment Voltage Sag Immunity {#voltage-sag-immunity}

- **SEMI F47** - Specification for Semiconductor Processing Equipment Voltage Sag Immunity

### Common Technical Standards for Machines and Mechanisms {#common-mm-ts}

- **S2-W-5** - Common Technical Standards for Machines and Mechanisms

### Guide to Documentation for Semiconductor Equipment Installation {#guide-equipment-installation}

- **SEMI E6** - Guide to Documentation for Semiconductor Equipment Installation

### Generic Immunity – Industrial Environments {#generic-immunity-industrial}

- **IEC 61000-6-2** - Generic immunity standard for industrial environments
- **EN 61000-6-2** - Generic immunity standard for industrial environments
- **KS C 9610-6-2** - Korean generic immunity standard for industrial environments

### Product Family Standards {#product-family-standards}

- **EN 50370-1** - Electromagnetic compatibility (EMC) - Product family standard for machine tools - Part 1: Emission
- **EN 50370-2** - Electromagnetic compatibility (EMC) - Product family standard for machine tools - Part 2: Immunity
- **EN 301 489-1/-3/-7/-9/-15/-17/-19/-24/-51/-52** - Product family standards for radio equipment and services

### Wi-Fi Devices Interoperability {#wifi-interoperability}

- **Wi-Fi CERTIFIED n** - Wi-Fi n certification
- **Wi-Fi Protected Setup** - Wi-Fi Protected Setup certification
- **WMM Power Save** - Wi-Fi Multimedia Power Save certification
- **Protected Management Frames** - Protected Management Frames certification
- **Miracast** - Miracast wireless display certification
- **Wi-Fi Direct** - Wi-Fi Direct certification
- **Wi-Fi CERTIFIED ac Test Plans** - Wi-Fi ac certification test plans