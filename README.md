...
タブを開くと認定書番号・認定取得組織・有効期限・PDFリンクをカードで表示します。認定を選択して別一覧を読み込む操作は不要です。一括照合後は認定・施設・検索語・判定で絞り込み、各行の「詳細を見る」からスコープの詳細とPDFを参照できます。集計は認定・施設・検索条件に連動し、判定フィルター適用前の件数を表示します。認定書の追加・更新ヘルプも同じタブに残しています。
