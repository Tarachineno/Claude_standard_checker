...
タブを開くと認定書番号・認定取得組織・有効期限・PDFリンクをカードで表示します。一括照合後は認定・施設・検索語・判定で絞り込み、集計カードからも判定フィルターを選べます。各行の「詳細を見る」からスコープの詳細とPDFを参照できます。集計は判定フィルター適用前の件数です。認定書の追加・更新は保守担当へ依頼する運用とし、画面内の更新ヘルプは削除しました。書式文書は保守用に保持しています。
...
- 定期巡回は外部スケジューラーで毎週日曜09:00（日本時間）。D1の現行スコープを重複排除し、公式出典を確認できた結果だけ反映します。旧Workerの15分Cronは停止しました。実行器に依存しない[週次巡回仕様](docs/publisher-review-runbook.md)と[結果JSONスキーマ](docs/publisher-review-result.schema.json)を使用します。
